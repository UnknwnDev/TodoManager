    ...
    def start(self):
        ...
        while True:
            ...
            if user_input in self.quit:
                ...
                if user_input in ["show", "show all"]:
                    if not self.manager.selected_category:
                        ...
                        try:
                            print(
                                f"Showing: {self.manager.selected_category.capitalize()}"
                            )
                            self.printf(
                                self.manager.get_tasks_by_category(
                                    self.manager.selected_category
                                )
                            )
                        except Exception:
                            print(
                                f"The List: {self.manager.selected_category} doesn't exist! Check spelling."
                            )
            ...
            # Task Managing
                ...
                ("delete task", "remove task", "remove", "delete")
            ):
                ...
                if len(task) < 2:
                    if len(task) > 1 and task[1].startswith("task"):
                        print(
                            "Please enter a task to remove. ( delete/remove task Clean )"
                        )
                    else:
                        if len(task) == 1:
                            print(
                                "Please enter a task to remove. ( delete/remove Clean )"
                            )
                        else:
                            print(task[-1].lower())
            ...
            # Selectors
                ...
                print(
                    f"You have de-selected {self.manager.selected_category.capitalize()}"
                )
